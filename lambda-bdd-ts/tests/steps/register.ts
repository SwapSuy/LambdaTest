...
//let page:Page;
  ...
  Given('Launch the Lambdatest web application', async function (this: CustomWorld) {
 ...
 await reg.goToU();
         ...
         When('click on Register Button', async function (this: CustomWorld) {
          ...
          Then('User should be on Register Account page', async function (this: CustomWorld) {
  ...
  expect(heading?.trim()).toBe('Register Account');
         ...
         Then('user enters values {string}, {string}, {string}, {string}, {string}, {string}', async function (this: CustomWorld,FirstName, LastName, EMail, Telephone, Password, PasswordConfirm) {
              let reg = new registerPage(this.page);
                // 🔄 Dynamically generate unique email address
    const uniqueEmail = `testuser_${Date.now()}@example.com`;
    console.log(`📧 Generated email: ${uniqueEmail}`);
              await reg.enterTextFields(FirstName,LastName,uniqueEmail,Telephone,Password,PasswordConfirm);
         });
